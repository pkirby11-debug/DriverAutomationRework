...
function Invoke-DATWebDownload {
    ...
    try {
        ...
        Write-Verbose "WebRequest download failed: $($_.Exception.Message)"
        return $false
    }
}
...
function Compress-DATPackage {
    ...
    $OutputDir = Join-Path (Split-Path $SourcePath -Parent) 'Compressed'
    ...
    switch ($CompressionType) {
        ...
        'WIM' {
            ...
            $WimTemp = Join-Path $env:TEMP 'DAT_WimTemp'
            if (Test-Path $WimTemp) { Remove-Item -Path $WimTemp -Recurse -Force }
            New-Item -Path $WimTemp -ItemType Directory -Force | Out-Null

            Write-DATLog -Message "Copying drivers to local temp for WIM creation: $WimTemp" -Severity 1
            Copy-Item -Path "$SourcePath\*" -Destination $WimTemp -Recurse -Force

            $LocalWim = Join-Path $WimTemp 'DriverPackage.wim'
            $DismArgs = "/Capture-Image /ImageFile:`"$LocalWim`" /CaptureDir:`"$WimTemp`" /Name:`"$PackageName`" /Compress:max"
            Write-DATLog -Message "DISM args: $DismArgs" -Severity 1
            $DismLog = Join-Path $WimTemp 'DismAction.log'
            ...
            # Clean up local temp
            Remove-Item -Path $WimTemp -Recurse -Force -ErrorAction SilentlyContinue
