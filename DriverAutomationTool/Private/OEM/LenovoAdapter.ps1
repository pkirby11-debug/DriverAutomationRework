...
function Get-LenovoBIOSUpdate {
    ...
    try {
        ...
        # Parse BIOS packages
        $BiosPackages = $Xml.SelectNodes('//Package')
        if (-not $BiosPackages -or $BiosPackages.Count -eq 0) {
